# src/reasoning_engine/core/tools.py (FINAL VERSION)
...
def extract_keywords(clauses: List[DocumentChunk], keywords: List[str]) -> List[Dict[str, Any]]:
    ...
    for chunk in clauses:
        if any(keyword.lower() in chunk.text.lower() for keyword in keywords):
             findings.append({"found_keyword": keywords[0], "evidence": chunk.text, "source": chunk.source_document, "page": chunk.page_label})
    return findings
...
def check_waiting_period(clauses: List[DocumentChunk], procedure_terms: List[str]) -> List[Dict[str, Any]]:
    ...
    for chunk in clauses:
        # Prioritize chunks that contain the specific procedure terms
        if any(term.lower() in chunk.text.lower() for term in procedure_terms):
            matches = waiting_period_pattern.finditer(chunk.text)
            for match in matches:
                findings.append({"waiting_period": match.group(0), "condition_context": chunk.text, "source": chunk.source_document, "page": chunk.page_label})
    # If no specific match, return the first generic waiting period found
    if not findings:
        for chunk in clauses:
            matches = waiting_period_pattern.finditer(chunk.text)
            for match in matches:
...
def extract_monetary_limit(clauses: List[DocumentChunk], limit_terms: List[str]) -> List[Dict[str, Any]]:
    ...
    findings = []
    monetary_pattern = re.compile(r'(Rs\.?|₹)\s?[\d,]+|\d+\s?(%|percent|lakh|lakhs|crore)\s*(of Sum Insured)?', re.IGNORECASE)
    for chunk in clauses:
        if any(term.lower() in chunk.text.lower() for term in limit_terms):
            matches = monetary_pattern.finditer(chunk.text)
            for match in matches:
                findings.append({"limit": match.group(0).strip(), "context": chunk.text, "source": chunk.source_document, "page": chunk.page_label})
    return findings
...
def find_information(clauses: List[DocumentChunk], topic: str) -> List[Dict[str, Any]]:
    ...
    if clauses:
        ...
        return [{"topic": topic, "evidence": clauses[0].text, "source": clauses[0].source_document, "page": clauses[0].page_label}]
    return []
